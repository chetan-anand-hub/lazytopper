...
import { outDir, repoRoot, runNodeScript } from "./persona_bot_lib.mjs";
...
async function readJourneyReport(journeyId) {
  const absPath = path.join(outDir, `browser_${journeyId}.json`);
  const raw = await fs.readFile(absPath, "utf8");
  return JSON.parse(raw);
}

async function loadJourneyReports() {
  const journeyIds = requiredJourneyIds();
  const reportsById = new Map();
  const missingIds = [];

  for (const journeyId of journeyIds) {
    try {
      reportsById.set(journeyId, await readJourneyReport(journeyId));
    } catch {
      missingIds.push(journeyId);
    }
  }

  if (missingIds.length > 0) {
    const res = runNodeScript("scripts/ops/browser_journeys/run_browser_journeys.mjs", [
      `--journeys=${missingIds.join(",")}`,
    ]);
    if (!res.ok) {
      throw new Error(`Browser journeys failed before persona mapping: ${res.stderr || res.stdout}`);
    }
    for (const journeyId of missingIds) {
      reportsById.set(journeyId, await readJourneyReport(journeyId));
    }
  }
...
async function main() {
  const reportsById = await loadJourneyReports();
  ...
  for (const bot of browserPersonaRules) {
    ...
    for (const requirement of bot.requirements) {
      const report = reportsById.get(requirement.journeyId);
      ...
      checks.push({
        ...
        details: ok
          ? `${requirement.journeyId} passed.`
          : `${requirement.journeyId} failed${failedNames.length ? ` (${failedNames.join(", ")})` : ""}.`,
  ...
  const outPath = path.join(outDir, "browser_persona_gate_audit.json");
  await fs.writeFile(
    outPath,
    JSON.stringify(
      {
