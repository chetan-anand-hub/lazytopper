...
try {
  ...
  const missingResolved = [...resolvedSelectorsPre].filter((sel) => (postCounts.get(sel) || 0) < 1);

  const duplicateSelectorsPre = new Set(
    (pre.duplicate_selector_blocks_identical || []).map((d) => String(d.selector || "")).filter(Boolean)
  );
  const missingDuplicateKeeps = [...duplicateSelectorsPre].filter((sel) => (postCounts.get(sel) || 0) < 1);
