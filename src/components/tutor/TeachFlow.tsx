...
function extractFeedbackText(payload: any): string {
  ...
  if (typeof payload.feedback === "string" && payload.feedback.trim()) return payload.feedback.trim();
  if (payload.teach?.checkpoint?.answer && typeof payload.teach.checkpoint.answer === "string")
    return payload.teach.checkpoint.answer.trim();
  if (typeof payload.text === "string" && payload.text.trim()) return payload.text.trim();
  if (typeof payload.data?.text === "string" && payload.data.text.trim()) return payload.data.text.trim();
...
function getGoal(card: TeachCard): string {
  return String(card.goal || card.goalLine || "").trim() || "Understand the core idea.";
}
...
const styles = {
  ...
  diagram: { border: "1px solid #e0e0e0", borderRadius: 8, marginBottom: 16 } as React.CSSProperties,
...
export function TeachFlow({ topicKey, subject, grade, onComplete }: TeachFlowProps) {
  ...
  async function startLearning() {
    ...
    try {
      ...
      if (card) {
        ...
        setTeachCard({ goal: `Let's learn ${topicKey}`, keyIdeas: ["Review the key concepts for this topic.", "Focus on definitions and theorems."], checkpoint: { question: "What do you already know about this topic?" } });
      }
  ...
  async function submitAnswer() {
    ...
    try {
      const payload = await callMentor({
        ...
        subject,
        grade,
        nodeId: `${topicKey}-step-${stepCount + 1}`,
        messages: newHistory,
  ...
  if (phase === "intro") {
    return (
      <div style={styles.container}>
        ...
        {error && <><div style={styles.error}>{error}</div><button style={styles.primaryBtn} onClick={retry}>Retry</button></>}
        {!loading && !error && (
  ...
  // PHASES: teaching, awaiting_answer, responding
    ...
    <div style={styles.container}>
      ...
      {card && (
        <div style={styles.card}>
          ...
          {diagram && (
            <svg width={200} height={100} viewBox="0 0 200 100" style={styles.diagram}>
              <rect width="200" height="100" fill="#f8f8f8"/>
              <text x="100" y="44" textAnchor="middle" fontSize={12} fill="#888">{diagram.type}</text>
              <text x="100" y="64" textAnchor="middle" fontSize={10} fill="#aaa">{(diagram.altText || "").slice(0, 40)}</text>
            </svg>
          )}
