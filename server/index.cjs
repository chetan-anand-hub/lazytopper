...
function buildSolveWithMeProtocolPrompt(payload) {
  ...
  return [
    ...
    '  "anchors": [ { "id": "A", "kind": "point|side|angle", "target": "A|BC|∠ABC", "label": "..." } ]',
    '  "diagramSteps": [ { "stepId": "s1", "highlightAnchorIds": ["A","BC"] } ]',
    '- IMPORTANT: Do NOT output SVG/ASCII art. Output only the spec above; the frontend will render.',
    '',
...
function buildBoardStepsMSPrompt(payload) {
  ...
  return [
    ...
    '  ],',
    '  "finalAnswer": "string",',
    '  // OPTIONAL (for geometry/figures):',
    '  "diagram": { "type": "triangle", "templateId": "triangle-basic", "payload": { ... } },',
    '  "anchors": [ { "id": "A", "kind": "point|side|angle", "target": "A|BC|∠ABC", "label": "..." } ],',
    '  "diagramSteps": [ { "stepId": "s1", "highlightAnchorIds": ["A","BC"] } ],',
    '  // IMPORTANT: No SVG. Frontend renders from the spec.',
