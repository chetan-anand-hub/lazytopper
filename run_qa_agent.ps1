...
Write-Host '[QA Agent] Checking @playwright/test...'
npm ls @playwright/test --depth=0 *> $null
if ($LASTEXITCODE -ne 0) {
  Write-Host '[QA Agent] @playwright/test not found. Installing Playwright silently...'
...
Set-Content -Path 'tests/student-bot.spec.ts' -Value $testSpec -Encoding UTF8

Write-Host '[QA Agent] Running headed Playwright test...'
npx playwright test tests/student-bot.spec.ts --headed --timeout=120000
